...
export function AddTransactionForm({ accounts, categories, editMode = false, initialData = null, }) {
    ...
    const { loading: transactionLoading, fn: transactionFn, data: transactionResult, } = useFetch(editMode ? updateTransaction : createTransaction); // Cast to unknown as useFetch is generic and its return type depends on the passed function
    const onSubmit = useCallback((data) => {
        const formData = Object.assign(Object.assign({}, data), { amount: parseFloat(data.amount) });
        if (editMode && editId) {
    ...
    useEffect(() => {
        if ((transactionResult === null || transactionResult === void 0 ? void 0 : transactionResult.success) && !transactionLoading) {
    ...
    return (<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <Select onValueChange={(value) => setValue("type", value, { shouldValidate: true })} defaultValue={type}>
          <SelectTrigger>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="space-y-2">
          ...
          <Input type="string" step="0.01" placeholder="0.00" {...register("amount")}/>
      ...
      <div className="space-y-2">
        ...
        <Select onValueChange={(value) => setValue("category", value)} defaultValue={getValues("category")}>
          ...
          <SelectContent>
            {filteredCategories.map((category) => (<SelectItem key={category.id} value={category.id}>
                {category.name}
              </SelectItem>))}
          </SelectContent>
        </Select>
      ...
      {isRecurring && (<div className="space-y-2">
          ...
          {errors.recurringInterval && (<p className="text-sm text-red-500">
